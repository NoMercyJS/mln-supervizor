...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        let sesteolmayan = message.guild.members.cache.filter(s => s.roles.cache.has('814911350358278156')).filter(s => !s.voice.channel).map(s => s).join(' ')
        let sesteolan = message.guild.members.cache.filter(s => s.roles.cache.has('814911350358278156')).filter(s => s.voice.channel).map(s => s).join(', ')
