...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        message.channel.send(embed.setDescription(`${member} Adlı Kullanıcı **${reason}** Sebebi ile **${time}** süre boyunca Jaile Atıldı.`)).then(x => x.delete({ timeout: 5000 }))
        ...
        channel.send(embed2.setTitle('Kullanıcı jaile Atıldı').setDescription(`Yetkili: ${message.author} - \`${message.author.id}\` \n Jaile Atılan: ${member} - \`${member.id}\` \n Sebep: ${reason} \n Süre: ${time} \n Tarih: **${moment().format('LLL')}**`))
        if (time) setTimeout(() => {
