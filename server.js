...
client.on('ready', () => {
    wait(1000);
    client.user.setPresence({ activity: { name: "† No Mercy Milona'yı Seviyor <3"}, status: 'online' })
...
client.rewards = [
  { role: "814911350358278158", ses: 72000, yazı: 1500 }
] // sesdeki 10000 = 10 saniye 

client.config = {
    token: '', //token
    banMembers: ['814911350328524890',"814911350328524887","814911350387507232","814911350375448613","814911350387507237","814911350387507238"], // Ban yetkilileri
    jailMembers: ['814911350328524889',"814911350328524887","814911350387507232","814911350375448613","814911350387507237","814911350387507238  "], // Jail yetkilileri
    jailRoles: ['814911350274129952'], // Jail rolleri
    muteMembers: ['814911350328524888',"814911350328524887","814911350387507232","814911350375448613","814911350387507237","814911350387507238"], // Yetkili rolleri
    muteRoles: ['814911350274129950'], // chat mute rolleri
    voicemuteRoles: ['814911350328524891',"814911350328524887","814911350387507232","814911350375448613","814911350387507237","814911350387507238"], // ses mute rolleri
    üstYönetim: ['814911350387507238',"814911350387507237","814911350375448613","814911350387507232"], //Üst Yönetim rolleri
    mesajLog: '815196375482236948', //Mesaj log
    vipRoles: ['809838149113502741299579955877953576'], //vip rolleri
    boosterRoles: '815545477290131487', //boosterrolü
    ekipRoles: ['814911350241362026'], //taglı rolü
    unregisteres: ['814911350207545368',"814911350207545367"], //kayıtsız rolü
    maleRoles: ['814911350241362021',"814911350241362020","814911350207545369"], //erkek rolleri
    girlRoles: ["814911350241362024","814911350241362023",'814911350241362022'], //kız rolleri
    mods: ['807571845464850452'], //mod rolleri
    chat: '814911352148852759', //chat idsi
    channelID: '814911350550822948', //kayıt kanalı id
    tag: '†', //tag
    guildID: '814911349817081907', //sunucu id
    taglog: '815196091921465345', //tag log id
//king royale:814911350387507238 - The Milona:814911350387507237 - Throne:814911350375448613 - Kayle:814911350387507232 - BanH:814911350328524890- BotCom:814911350328524887
//jailH:814911350328524889 - MuteH:814911350328524888 - V.MuteH:814911350328524891 - RegH:814911350358278155 - Transport:814911350328524886
}
...
let white = {// , koyup aynı şekilde devam ettirebilirsiniz
"512273192522612775": true,
"589079197906108416": true,
"773937985787658240": true,
"747397256054964274": true
};
...
client.on('message', (message) => {
  if(message.attachments.size >= 1){
   if(message.channel.id == "814911352148852759"){//chat kanalı
...
client.on('messageDelete', (message) => {
  let channel = client.guilds.cache.get(client.config.guildID).channels.cache.find(c => c.name === "message-log") //log kanalının ismi
...
client.on("userUpdate", async function(oldUser, newUser) {
  ...
  if (newUser.username !== oldUser.username) {
      if (oldUser.username.includes(client.config.tag) && !newUser.username.includes(client.config.tag)) {
          ...
          client.channels.cache.get(client.config.chat).send(`Tebrikler, ${newUser} tag alarak ailemize katıldı ! \`(${client.config.tag})\``)
...
client.on('guildMemberAdd', (member) => {
  const mapping = {
      "0": "<a:mln_0:815216309394604034>", // Sayı İdlerini <a:isim:id> şeklinde giriniz.
  ...
  if (memberDay > 604800000) {
      client.channels.cache.get(client.config.channelID).send(`<a:mln_siyahhac:815893234668011550> Suncumuza hoşgeldin ${member} - \`${member.id}\`,
      <a:mln_beyazhac:815893218054242334>    Seninle birlikte **${toplamüye}** üyeye ulaştık,
      <a:mln_siyahhac:815893234668011550>       Hesabın **${createAt}** önce açılmış,
      <a:mln_beyazhac:815893218054242334>    Kayıt olmak için tag alman/boost basman gerek,
      <a:mln_siyahhac:815893234668011550> Seni davet eden kişi ${daveteden} **(${davetsayi} Davet)** !`)
  } else {
      client.channels.cache.get(client.config.channelID).send(
          new Discord.MessageEmbed()
          .setAuthor(member.user.username, member.user.avatarURL({ dynamic: true }))
          .setDescription(`${member}, Adlı Kullanıcı Sunucuya Katıldı Hesabı **${createAt2}** Önce Açıldığı İçin Şüpheliye Atıldın!`)
          .setTimestamp()
